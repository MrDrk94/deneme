...
public static void sayýüret(){
	domates=(int)(Math.random()*3);
	mýsýr=(int)(Math.random()*tarlagenisligi/2+1);
	fasulye=(int)(Math.random()*tarlagenisligi/3)+1;
	
	while(2*domates+2*mýsýr+3*fasulye>tarlagenisligi)
		{
			domates=(int)(Math.random()*3);
			mýsýr=(int)(Math.random()*tarlagenisligi/2+1);
			fasulye=(int)(Math.random()*tarlagenisligi/3)+1;
...
public static void main(String[] args) {
	System.out.println("Tarla genisliðini giriniz");
		...
		System.out.println("Lütfen baþlangýç deðerlerinizi kýsýtlara dikkat ederek veriniz!!!");
		System.out.println("(Domates en fazla 2, fasulye en az 1 olmalýdýr :)  )");
